...
build_install() {
  if [ ! -d "${PKG_BUILD}"/install ]; then
    ...
    for filename in *; do
      ln -sf /usr/lib/aarch64-linux-gnu/tegra/${filename} ../../${filename}
    done
    cd ../tegra-egl
    for filename in *; do
      ln -sf /usr/lib/aarch64-linux-gnu/tegra-egl/${filename} ../../${filename}
    done
    cd ..
    for filename in *; do
      ln -sf /usr/lib/aarch64-linux-gnu/${filename} ../../${filename}
    done
    cd ../../../
    pwd
...
makeinstall_target() {
  ...
  PWD=$(pwd)
  cd install/usr/lib/aarch64-linux-gnu/tegra
  rm ${TOOLCHAIN}/aarch64-libreelec-linux-gnueabi/sysroot/usr/lib/{libGLX_nvidia.so.0,libcuda.so.1.1,libdrm.so.2,libnvapputil.so,libnvargus.so,libnvargus_socketclient.so,libnvargus_socketserver.so,libnvavp.so,libnvbuf_fdmap.so.1.0.0,libnvbuf_utils.so.1.0.0,libnvbufsurface.so.1.0.0,libnvbufsurftransform.so.1.0.0,libnvdsbufferpool.so.1.0.0,libnvid_mapper.so.1.0.0,libnvcam_imageencoder.so,libnvcameratools.so,libnvcamerautils.so,libnvcamlog.so,libnvcamv4l2.so,libnvcolorutil.so,libnvdc.so,libnvddk_2d_v2.so,libnvddk_vic.so,libnveglstream_camconsumer.so,libnveglstreamproducer.so,libnveventlib.so,libnvexif.so}
  echo loop1
  for filename in *; do
   #if [ ! "${filename}" = "ld.so.conf" -o ! -d "${filename}" ]; then
      if [ -h "${TOOLCHAIN}/aarch64-libreelec-linux-gnueabi/sysroot/usr/lib/${filename}" ]; then
        rm -rf ${TOOLCHAIN}/aarch64-libreelec-linux-gnueabi/sysroot/usr/lib/${filename}
      else
        cp -PRvnf ${filename} ${TOOLCHAIN}/aarch64-libreelec-linux-gnueabi/sysroot/usr/lib/
      fi
   #fi
  done
  cd ../tegra-egl
  ...
  #rm libGLX_nvidia.so.0
  #rm libv4lconvert.so.0 libv4lconvert.so libv4l2.so.0 libv4l2.so
  #rm libnvbuf_utils.so
  #cp libnvbuf_utils.so.1.0.0 libnvbuf_utils.so
  #mv libnvv4l2.so.0 libv4l2.so.0
